...
class khaligefars extends Command
{
    private $season;
     ...
     * @var string
     */
    protected $signature = 'command:khaligefars  {season=null}';
    ...
    public function handle()
    {
        ...
        $leagueService->table(
            LeaguesConstants::KHALIGEFARS,
            $this->season,
            LeaguesConstants::$Source[LeaguesConstants::VARZESH3]
        );
        $standing = $leagueService::$table;
        $this->standingDatabase('khaligefars_table', $standing, $this->season);

        $this->info("{$this->season}-scorers");
        $scorers = $leagueService->navad(
            LeaguesConstants::$Navad['KHALIGEFARS'],
            LeaguesConstants::$Navad['SCORERS']
        );
        $this->scorersDatabase('khaligefars_scorers', $scorers, $this->season);

        $this->info("{$this->season}-fixtures");
        $this->renderUrl(470, 'khaligefars');
    ...
    private function scorersDatabase($table, $scorers, $season)
    {
        foreach ($scorers as $scorer) {
            DB::table($table)
              ->updateOrInsert([
                  'name'   => $scorer['name'],
                  'club'   => $scorer['team_name'],
                  'season' => $season,
              ], [
                  'name'         => $scorer['name'],
                  'club'         => $scorer['team_name'],
                  'count_scores' => $scorer['goals'],
