...
class europeLeague extends Command
{
    private $season;
     ...
     * @var string
     */
    protected $signature = 'command:europe-league  {season=null}';
    ...
    public function handle()
    {
        ...
        $path = DB::table('scoreboard')
                  ->where('tournament', 'EUROPE_LEAGUE')
        ...
        $this->standingDatabase('europe_league_table', $standing, $this->season);

        $this->info("{$this->season}-scorers");
        $scorers = $leagueService->scoreboard($path->scorers, 'scorers', TRUE);
        $this->scorersDatabase('europe_league_scorers', $scorers, $this->season);
    }
