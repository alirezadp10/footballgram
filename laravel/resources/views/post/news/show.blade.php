...
@endsection
@section('styles')
    <link href="{{ mix('css/show-news.css') }}" rel="stylesheet">
    @if($response['accessToManageChiefChoice'])
        <meta name="chief-choice" content="true">
    @endif
    @if($response['accessToManageSlidePost'])
        <meta name="slide-post" content="true">
    @endif
...
@section('content')
    <div class="post-main-header">
        ...
        @auth
            @if($response['accessToManageSlidePost'] || $response['accessToManageChiefChoice'] || $response['accessToDelete'] || $response['accessToEdit'])
            ...
            <div class="dropdown-menu post-action-dropdown-menu"
                ...
                @if($response['accessToEdit'])
                    <a class="dropdown-item"
                       href="{{ route('news.edit',$response['slug']) }}">ویرایش</a>
                @endif
                @if($response['accessToManageSlidePost'])
            ...
            @if($response['accessToDelete'])
                <form hidden
                      id="delete-post"
                      method="post"
                      action="{{ route('news.destroy',$response['slug']) }}">
                    @csrf
                    @method('delete')
                </form>
            @endif
...
@endsection
@section('scripts')
    <script src="{{ mix('js/show-news.js') }}"></script>
@endsection
