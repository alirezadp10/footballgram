...
class europeNationsLeague extends Command
{
    private $season;
     ...
     * @var string
     */
    protected $signature = 'command:europe-nations-league  {season=null}';
    ...
    {
        $this->info('europe-nations-league');
        ...
        $path = DB::table('scoreboard')
                  ->where('tournament', 'EUROPE_NATIONS_LEAGUE')
        ...
        $this->standingDatabase('europe_nations_league_table', $standing, $this->season);

        $this->info("{$this->season}-scorers");
        $scorers = $leagueService->scoreboard($path->scorers, 'scorers', TRUE);
        $this->scorersDatabase('europe_nations_league_scorers', $scorers, $this->season);
    }
