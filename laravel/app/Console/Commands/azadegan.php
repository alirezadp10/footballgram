...
class azadegan extends Command
{
    private $season;
     ...
     * @var string
     */
    protected $signature = 'command:azadegan  {season=null}';
    ...
    public function handle()
    {
        ...
        $leagueService->table(
            LeaguesConstants::AZADEGAN,
            $this->season,
            LeaguesConstants::$Source[LeaguesConstants::VARZESH3]
        );
        $standing = $leagueService::$table;
        $this->standingDatabase('azadegan_table', $standing, $this->season);

        $this->info("{$this->season}-scorers");
        $scorers = $leagueService->navad(
            LeaguesConstants::$Navad['AZADEGAN'],
            LeaguesConstants::$Navad['SCORERS']
        );
        $this->scorersDatabase('azadegan_scorers', $scorers, $this->season);

        $this->info("{$this->season}-fixtures");
        $this->renderUrl(498, 'azadegan');
