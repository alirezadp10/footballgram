...
class UserContent extends Model
{
    ...
    public static function getUserContentWithComments($slug)
    {
    ...
    public static function getUserContentWithoutComments($slug)
    {
        return DB::select("SELECT 
                              post.id,
                              post.slug,
    ...
    public static function getUserContentForComposer($take = 16, $offset = 0)
    {
        return
            DB::table('user_contents')
              ->select(
                  'user_contents.id',
                  'user_contents.main_title',
                  'user_contents.secondary_title',
                  'user_contents.slug',
                  'user_contents.like',
                  'user_contents.dislike',
                  'images.md as image',
                  'users.id as author_id',
                  'users.name',
                  DB::raw('COALESCE(GROUP_CONCAT(distinct CONCAT(likes.user_id)), "") AS users_liked'),
                  DB::raw('COALESCE(GROUP_CONCAT(distinct CONCAT(dislikes.user_id)), "") AS users_disliked')
              )
