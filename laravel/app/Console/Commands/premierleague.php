...
class premierleague extends Command
{
    private $season;
     ...
     * @var string
     */
    protected $signature = 'command:premierleague  {season=null}';
    ...
    public function handle()
    {
        ...
        $leagueService->table(
            LeaguesConstants::PREMIERLEAGUE,
            $this->season,
            LeaguesConstants::$Source[LeaguesConstants::VARZESH3]
        );
        $standing = $leagueService::$table;
        $this->standingDatabase('premier_league_table', $standing, $this->season);

        $this->info("{$this->season}-scorers");
        $path = DB::table('scoreboard')
                  ->where('tournament', 'PREMIERLEAGUE')
                  ->pluck('scorers')
                  ->first();
        $scorers = $leagueService->scoreboard($path,'scorers');
        $this->scorersDatabase('premier_league_scorers', $scorers, $this->season);

        $this->info("{$this->season}-fixtures");
        $this->renderUrl(484, 'premier_league');
