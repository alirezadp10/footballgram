...
class loshampione extends Command
{
    private $season;
     ...
     * @var string
     */
    protected $signature = 'command:loshampione  {season=null}';
    ...
    public function handle()
    {
        ...
        $leagueService->table(
            LeaguesConstants::LOSHAMPIONE,
            $this->season,
            LeaguesConstants::$Source[LeaguesConstants::VARZESH3]
        );
        $standing = $leagueService::$table;
        $this->standingDatabase('loshampione_table', $standing, $this->season);

        $this->info("{$this->season}-scorers");
        $path = DB::table('scoreboard')
                  ->where('tournament', 'LOSHAMPIONE')
                  ->pluck('scorers')
                  ->first();
        $scorers = $leagueService->scoreboard($path,'scorers');
        $this->scorersDatabase('loshampione_scorers', $scorers, $this->season);

        $this->info("{$this->season}-fixtures");
        $this->renderUrl(486, 'loshampione');
